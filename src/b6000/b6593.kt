    ...
    while(isContinue) {
        ...
        if (l == 0 && r == 0 && c == 0) {
            ...
            while(que.isNotEmpty()) {
                ...
                listOf(
                    {x:Int, y:Int, z:Int, time:Int -> State(x + 1, y, z, time + 1)},
                    {x:Int, y:Int, z:Int, time:Int -> State(x - 1, y, z, time + 1)},
                    {x:Int, y:Int, z:Int, time:Int -> State(x, y + 1, z, time + 1)},
                    {x:Int, y:Int, z:Int, time:Int -> State(x, y - 1, z, time + 1)},
                    {x:Int, y:Int, z:Int, time:Int -> State(x, y, z + 1, time + 1)},
                    {x:Int, y:Int, z:Int, time:Int -> State(x, y, z - 1, time + 1)}
                ).forEach {
                    val moved = it(now.x, now.y, now.z, now.time)
